...
void diviofaandb(int *f,int *g,int n,int m)
{
	static int d[N<<1],r[N<<1];
	int L=n-m+1;
	reverse(f,f+n);cpy(r,f,L);reverse(f,f+n);
	reverse(g,g+m);cpy(d,g,L);reverse(g,g+m);
	get_invoff(d,L);times(d,r,L,L);reverse(d,d+L);
	times(g,d,n,n);
	R(i,0,m-2) g[i]=(f[i]-g[i]+mod)%mod;
	clr(g+m-1,L);
	cpy(f,d,L);clr(f+L,n-L);
}
signed main()
{
	n=read()+1,m=read()+1;
	...
	R(i,0,m-1) B[i]=read();
	diviofaandb(A,B,n,m);
	R(i,0,n-m) printf("%lld ",A[i]);puts("");
	R(i,0,m-2) printf("%lld ",B[i]);
}
