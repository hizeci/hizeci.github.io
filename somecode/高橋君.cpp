...
signed main()
{
	...
	R(i,1,n)
	{
		while(L<qs[i].l) res=(res+res-C(L++,R)+mod)%mod;
		while(L>qs[i].l) res=(res+C(--L,R)),res>=mod?res-=mod:1,res=res*inv[2]%mod;
		while(R<qs[i].r) res=(res+C(L,++R)),res>=mod?res-=mod:1;
		while(R>qs[i].r) res=(res-C(L,R--)+mod)%mod;
		ans[qs[i].id]=res;
	}
	R(i,1,n) printf("%lld\n",ans[i]);
}