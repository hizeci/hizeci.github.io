...
1.连续两次 Access 操作时，第二次 Access 操作的返回值等于这两个节点的 LCA.

2.表示x 到根的链所在的 Splay 树的根。这个节点一定已经被旋转到了根节点，且父亲一定为空。
*/

inline void make_root(int x){
	x=access(x),push_flp(x);
}
/*
inline void make_root(int x){
...
/*
inline void cut(int x,int y){//不行!!
	make_root(x),access(y),splay(y);
	if(fa[x]==y&&find_root(x)==y&&!son[x][1]){
		son[y][0]=fa[x]=0;
		push_up(y);
	}
}
