...
inline int build(int k){
	key[++tot_nd]=k,siz[tot_nd]=1,wei[tot_nd]=rand1();
	return tot_nd;
}
int mer(int u,int v)
{
	...
	{
		if(tag[v]) push_down(v);
		son[v][0]=mer(x,son[v][0]);
		maintain(v);
		return v;
	}
}
...
{
	n=read(),m=read();
	R(i,1,n) rt=mer(rt,build(i));
	R(i,1,m)
	{
		int l=read(),r=read();
		pai t=spilt(rt,l-1);
		pai s=spilt(t.b,r-l+1);
		tag[s.a]^=1;
		rt=mer(t.a,mer(s.a,s.b));
	}
	print(rt);
}