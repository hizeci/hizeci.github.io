...
{
	int key,val,fa,son[2];
	Cartesian_node (int _k=0,int _v=0,int _f=0,int s1=0,int s2=0){key=_k,val=_v,fa=_f,son[0]=s1,son[1]=s2;}
	inline int operator <(Cartesian_node const &a)const{ return key<a.key;}
}tr[N];
...
int dfs(int u) {
	if(!u) return 0;
	int sz=dfs(tr[u].son[0])+dfs(tr[u].son[1])+1;
	ckmax(ans,1ll*sz*tr[u].val);
	return sz;
}
