...
signed main()
{
	...
	{
		cnt=tot=0;
		while((int)q.size()>0) tmp[++cnt]=q.top(),q.pop();
		R(i,1,m)
			L(j,1,cnt)
			{
				if(tot<k) q.push(a[fff][i]+tmp[j]),++tot;
				else if(q.top()>a[fff][i]+tmp[j]) q.pop(),q.push(a[fff][i]+tmp[j]);				
				else break;
			}
	}
	cnt=0;
	while((int)q.size()>0) tmp[++cnt]=q.top(),q.pop();
	L(i,1,cnt) writesp(tmp[i]);
}