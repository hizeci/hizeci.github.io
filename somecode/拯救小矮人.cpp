...
*/
int n,h,dp[2222];
struct node{int x,y,xy;}a[2222];
signed main()
{
	n=read();int x,y;
	R(i,1,n) x=read(),y=read(),a[i]=(node){x,y,x+y};h=read();
	sort(a+1,a+n+1,[&](node x,node y){return x.xy<y.xy;});
	R(i,1,n) dp[i]=-inf,dp[0]+=a[i].x;
	R(i,1,n) L(j,1,i) if(dp[j-1]+a[i].y>=h) {ckmax(dp[j],dp[j-1]-a[i].x);}
	L(i,0,n) if(dp[i]>=0) {writesp(i);return 0;} 
}