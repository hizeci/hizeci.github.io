...
{	
	...
	R(i,1,n) {
		modify_add(i);
		//for(int j=1;j<=10;j++) printf("ffa:%lld siz_hson:%lld Ls:%lld Rs:%lld siz_softchain:%lld lazy:%lld val:%lld\n",ffa[j],siz_hson[j],Ls[j],Rs[j],siz_softchain[j],lazy[j],val[j]); 
		for(auto qwq:qs1[i]) 
			ans[qwq]-=query(pos[qwq]);
		for(auto qwq:qs2[i]) 
			ans[qwq]+=query(pos[qwq]);
	}
	FR(i,0,q) {
