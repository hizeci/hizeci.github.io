...
*/
int n;
struct node{int a,b,opt,id;inline bool operator <(const node &A)const{return (opt^A.opt)?opt<A.opt:opt<=0?a<A.a:b>A.b;} }nd[11111];
...
signed main()
{
	...
	R(i,1,n) {
		nd[i].b=read();
		int opt=nd[i].a-nd[i].b;
		if(opt<0) nd[i].opt=-1;
		else if(!opt) nd[i].opt=0;
		else nd[i].opt=1;
		nd[i].id=i;
	}
	sort(nd+1,nd+n+1);
	int ta,tb;
	ta=nd[1].a,tb=nd[1].a+nd[1].b;
	R(i,2,n) tb=max(ta+nd[i].a,tb)+nd[i].b,ta+=nd[i].a;
	writeln(tb);
	R(i,1,n) writesp(nd[i].id);
}