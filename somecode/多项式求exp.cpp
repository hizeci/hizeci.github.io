...
{
	inv[1]=1;
	R(i,2,lim) inv[i]=1ll*inv[mod%i]*(mod-mod/i)%mod;
}
void derivation(int *f,int m)
{
	R(i,1,m-1) f[i-1]=1ll*f[i]*i%mod;
	f[m-1]=0;
}
void integral(int *f,int m)
{
	L(i,1,m) f[i]=1ll*f[i-1]*inv[i]%mod;
	f[0]=0;
}
void get_lnoff(int *f,int m)
{
	static int g[N<<1];
	cpy(g,f,m);
