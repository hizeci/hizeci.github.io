...
uniform_int_distribution<long long> dist(0, 10000000);  // ???¡§¡¤??¡ì
...
signed main(){
	...
	a[0]=-inf,a[n+1]=inf;
	R(i,1,m) {
		int x=read(),res=a[1]+a[2];
		R(j,1,n) {
			int ned=x-a[j];
			int tmp=upb(a,a+n+2,ned)-a;
			int l=max(0ll,tmp-5),r=min(n,tmp+5);
			R(k,l,r) {
				if(k==j) continue;
				if(abs(ned-a[k])<abs(res-x)) {
					res=a[k]+a[j];
				}
			}
