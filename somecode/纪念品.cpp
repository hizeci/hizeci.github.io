...
int ans;
int t,n,m;
int a[111][111],dp[11111];

signed main()
{
	freopen("souvenir.in","r",stdin);
	freopen("souvenir.out","w",stdout);
	t=read(),n=read(),ans=m=read();
	R(i,1,t) R(j,1,n) a[i][j]=read();
	R(i,1,t-1)
	{
		fill(dp,dp+ans+5,-inf);
		dp[ans]=ans;
		R(j,1,n) L(k,a[i][j],ans) ckmax(dp[k-a[i][j]],dp[k]+a[i+1][j]-a[i][j]);
		int mx=-1;
		R(j,0,ans) ckmax(mx,dp[j]);
		ans=mx;
	}
	writeln(ans);
}	