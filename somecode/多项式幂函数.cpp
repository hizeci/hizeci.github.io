...
inline int read_mod()
{
	char c=getchar();int x=0;bool f=0;
    for(;!isdigit(c);c=getchar())f^=!(c^45);
    for(;isdigit(c);c=getchar())x=(x<<1)+(x<<3)+(c^48),x%=mod;
    if(f)x=-x;
    return x;
}
...
inline void init_inv(int lim){inv[1]=1;R(i,2,lim) inv[i]=1ll*inv[mod%i]*(mod-mod/i)%mod;}
inline void derivation(int *f,int m){R(i,1,m-1) f[i-1]=1ll*f[i]*i%mod;f[m-1]=0;}
inline void integral(int *f,int m){L(i,1,m)f[i]=1ll*f[i-1]*inv[i]%mod;f[0]=0;}
void get_ln(int *f,int m)
{
	static int g[N<<1];
	cpy(g,f,m);
	get_invoff(g,m);derivation(f,m);
	times(f,g,m,m);
	integral(f,m-1);
	clr(g,m);
}
void get_exp(int *f,int m)
{
	static int s[N<<1],s2[N<<1];
	int n;for(n=1;n<m;n<<=1);
	s2[0]=1;
	for(int l=2;l<=n;l<<=1)
	{
		cpy(s,s2,l>>1);get_ln(s,l);
		R(i,0,l-1) s[i]=(f[i]-s[i]+mod)%mod;s[0]=(s[0]+1)%mod;
		times(s2,s,l,l);
	}cpy(f,s2,m);clr(s,n);clr(s2,n);
}
void polypow(int *f,int n,int k)
{
	int hada=0,cnt0=0;
	static int sav[N<<1];
	R(i,0,n-1) !(f[i]||hada)?cnt0++:hada=1,sav[i-cnt0]=f[i];
	int tmp=sav[0];
	get_ln(sav,n-cnt0);
	R(i,0,n-cnt0-1) sav[i]=sav[i]*k%mod;
	get_exp(sav,n-cnt0);
	cnt0*=k;tmp=fpow(tmp,k,mod);
	clr(f,n);
	R(i,cnt0,n-1) f[i]=sav[i-cnt0]*tmp%mod;
	clr(sav,n);
}
signed main()
{
